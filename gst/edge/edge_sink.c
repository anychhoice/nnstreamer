...
gst_edgesink_class_init (GstEdgeSinkClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_HOST,
      g_param_spec_string ("host", "Host",
          "A host address to send the packets to edgesrc", DEFAULT_HOST,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  g_object_class_install_property (gobject_class, PROP_PORT,
      g_param_spec_uint ("port", "Port",
          "A port address to send the packets to edgesrc (0=random available port)",
...
gst_edgesink_start (GstBaseSink * basesink)
{
  ...
  ret =
      ...
      NNS_EDGE_FLAG_SEND | NNS_EDGE_FLAG_SERVER, &self->edge_h);
...
gst_edgesink_render (GstBaseSink * basesink, GstBuffer * buffer)
{
  ...
  nns_edge_publish (self->edge_h, data_h);
  goto done;
